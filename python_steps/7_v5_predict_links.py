...
gat_v5_file=os.path.join(models_dir, 'gat_v5.pt')
mutant_embedding_file=os.path.join(new_data_dir, 'g2019s_protein_embedding.pt')
results_output_file=os.path.join(results_dir, 'v5_link_prediction_results.csv')
...
class link_predictor(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels_one, out_channels, heads):
        ...
        self.decoder_lin1 = torch.nn.Linear(out_channels*2, out_channels)
        self.decoder_lin2 = torch.nn.Linear(out_channels, 1)
        self.decoder_drop = torch.nn.Dropout(p=0.3)
    ...
    def decode(self, z, edge_label_index):
        ...
        fwd = self.decoder_lin2(
            self.decoder_drop(
                self.decoder_lin1(torch.cat([src, dst], dim=1)).relu()
            )
        ).squeeze()
        bwd = self.decoder_lin2(
            self.decoder_drop(
                self.decoder_lin1(torch.cat([dst, src], dim=1)).relu()
            )
        ).squeeze()
        return (fwd + bwd) / 2
...
if __name__=="__main__":
    ...
    model.load_state_dict(torch.load(gat_v5_file,map_location=device))
    model.eval()
